...
def update_conversation(id: int, user_message: str, llm_message: str,
        ...
        with psycopg2.connect(**conn_params) as conn:
            with conn.cursor() as cur:
                insert_query = sql.SQL("""
                        ...
                        where id =%s;
                    """)
                cur.execute(insert_query, (user_message, llm_message, time, id))
                conn.commit()
...
class User:  # 后期需要修改file_path为上传文件
    """
    用户类
    """

    def __init__(self, user_id: str = '', user_name: str = '', pw: str = '', file_path: str = '',
                 is_file_analysis: bool = False):
        self.is_file_analysis = is_file_analysis
        self._document_content_id = []  # 文档解析结果id，用于数据库查询
    ...
    def save_document(self, content: str):
        ...
        try:
            # 在用户表添加content
            logger.info("[INFO] users table: Data to be stored:")  # 打印待存储的数据
            conn_params = {
                "host": "192.168.143.117",
                "port": "9002",
                "database": "history_message",
            ...
            with psycopg2.connect(**conn_params) as conn:
                with conn.cursor() as cur:
                    update_query = sql.SQL("""
                                    UPDATE  users 
                                    SET file_content = array_append(file_content, %s)
                                    where id=%s;
                                    """)
                    cur.execute(update_query, (content, self.user_id))
                    logger.info(
                        f"[INFO] users table add file content successfully.")  # 打印用户表更新成功日志
            ...
            with psycopg2.connect(**conn_params) as conn:
                with conn.cursor() as cur:
                    ...
                    logger.info(
                        f"[INFO] document_content table updated {len(data_list)} lines successfully.")  # 打印用户表更新成功日志
    ...
    @document_content_id.setter
    def document_content_id(self, id_list):
        self._document_content_id = id_list


if __name__ == '__main__':
    user = User('123456', file_path='inputs/Chapter2社会网络的基础知识.pdf', is_file_analysis=True)
