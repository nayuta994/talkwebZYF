...
def parse_doc(
    ...
    """
        ...
        lang: 语言选项，默认为 'ch'，可选值包括['ch', 'ch_server', 'ch_lite', 'en', 'korean', 'japan', 'chinese_cht', 'ta', 'te', 'ka']。
            如果知道PDF中的语言，输入语言可以提升OCR的准确性。此选项为可选。
            仅适用于当后端设置为 "pipeline" 时。
        backend: 解析PDF的后端：
            pipeline: 更通用。
            vlm-transformers: 更通用。
            vlm-sglang-engine: 更快（引擎模式）。
            vlm-sglang-client: 更快（客户端模式）。
            如果没有指定方法，默认使用 pipeline。
        method: 解析PDF的方法：
            auto: 根据文件类型自动确定方法。
            txt: 使用文本提取方法。
            ...
            如果没有指定方法，默认使用 'auto'。
            仅适用于当后端设置为 "pipeline" 时。
        server_url: 当后端是 `sglang-client` 时，需要指定 server_url，例如：`http://127.0.0.1:30000`
        start_page_id: 解析的起始页ID，默认为 0
        end_page_id: 解析的结束页ID，默认为 None（解析文档直到最后一页）
    """
    try:
        file_name_list = []
        pdf_bytes_list = []
...
if __name__ == '__main__':
    ...
    pdf_files_dir = os.path.join(__dir__, "inputs")
    output_dir = os.path.join(__dir__, "outputs")
    pdf_suffixes = [".pdf"]
